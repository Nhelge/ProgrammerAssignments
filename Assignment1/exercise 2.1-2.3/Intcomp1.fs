...
(* Exercise 2.1: udvidet multiple sequential let-bindings*)
type expr = 
  | CstI of int
  | Var of string
  | Let of (string * expr) list * expr (* ændret *)
...
(* Exercise 2.1: eval ændret så den virker med den nye expr*)
...
let rec eval e (env : (string * int) list) : int =
    ...
    | Let(binding, ebody) -> 
      let env1 = 
        List.fold (fun accEnv (x, erhs) ->
            let xval = eval erhs accEnv
            (x, xval) :: accEnv)
          env binding 
      eval ebody env1
