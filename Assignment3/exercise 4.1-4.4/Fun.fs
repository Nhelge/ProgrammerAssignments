...
(* Exercise 4.3: Changed string to string list to match the new Absyn.fs*)
type value = 
  | Int of int
    ...
    | Call(Var f, eArgs) -> 
      ...
      | Closure (f, xs, fBody, fDeclEnv) ->
        if List.length xs <> List.length eArgs then
            failwith "eval Call: wrong number of arguments"
...
(* Exercise 4.3: argument wrapped in lists*)
